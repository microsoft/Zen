...
namespace ZenLib.ModelChecking
{
    ...
    internal static class StateSetTransformerFactory
    {
        ...
        public static StateSetTransformer<T1, T2> CreateTransformer<T1, T2>(Func<Zen<T1>, Zen<T2>> function, StateSetTransformerManager manager)
        {
            ...
            var input = Zen.Arbitrary<T1>(generator, 0, false);
            ...
            var output = Zen.Arbitrary<T2>(generator, 0, false);
            ...
            var symbolicEvaluator = new SymbolicEvaluationVisitor<Assignment<BDDNode>, Variable<BDDNode>, DD, BitVector<BDDNode>, Unit, Unit, Unit, BitVector<BDDNode>>(solver);
            var env = new SymbolicEvaluationEnvironment<Assignment<BDDNode>, Variable<BDDNode>, DD, BitVector<BDDNode>, Unit, Unit, Unit, BitVector<BDDNode>>(arguments);
            var symbolicValue = symbolicEvaluator.Evaluate(newExpression, env);
            var symbolicResult = (SymbolicBool<Assignment<BDDNode>, Variable<BDDNode>, DD, BitVector<BDDNode>, Unit, Unit, Unit, BitVector<BDDNode>>)symbolicValue;
        ...
        public static StateSet<T> CreateStateSet<T>(Func<Zen<T>, Zen<bool>> function, StateSetTransformerManager manager)
        {
            // create an arbitrary input and invoke the function
            var generator = new SymbolicInputGenerator();
            var input = Zen.Arbitrary<T>(generator, 0, false);
            var arbitrariesForInput = generator.ArbitraryExpressions;
            ...
            object zenExpr)
        {
            ...
            if (zenExpr is Zen<char>)
            {
                var (v, _) = solver.CreateCharVar(zenExpr);
                zenExprToVariable[zenExpr] = v;
                return;
            }
